...
const Service = memo(
  ({ slice, currentService, index, firstServiceRef, contextSafe }) => {
    const { uid } = slice;
    ...
    return (
      <article
        ...
        className={clsx(
          ...
          ((!currentService && index === 0) || currentService === uid) &&
            "active",
        )}
      >
          ...
          <div className="flex flex-col mb-8 md:flex-row">
            ...
            <div className="flex flex-col w-full md:max-w-[33.3125rem] mt-16 md:mt-0 md:ml-[7.0625rem]">
              <PrismicRichText
                components={{
                  heading1: ({ children }) => (
                    <h1 className="text-[2rem] font-bold italic mb-4">
                      {children}
                    </h1>
                  ),
                  paragraph: ({ children }) => (
                    <p className="text-[2rem] md:text-[1.125rem] mb-8">
                      {children}
                    </p>
                  ),
                }}
              ...
              <PrismicRichText
                components={{
                  ...
                  paragraph: ({ children }) => (
                    <p className="text-[2rem] md:text-[1.125rem]">{children}</p>
                  ),
                }}
